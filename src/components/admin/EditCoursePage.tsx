...
import { updateCourse, type Course } from '../../lib/queries'
...
export default function EditCoursePage({ course, enrollmentCount, onUpdated, onBack }: EditCoursePageProps) {
  const [title, setTitle] = useState(course.title)
  const [subtitle, setSubtitle] = useState(course.subtitle)
  const [description, setDescription] = useState(course.description)
  const [modality, setModality] = useState<'virtual' | 'presencial'>(course.modality)
  const [published, setPublished] = useState(course.published)
  const [maxEnrollments, setMaxEnrollments] = useState(course.max_enrollments?.toString() ?? '')
  ...
  const [locationName, setLocationName] = useState(course.location_name ?? '')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      sileo.success({ title: 'Curso actualizado', description: `"${title.trim()}" fue actualizado` })
      onUpdated()
    } catch {
      sileo.error({ title: 'Error', description: 'No se pudo actualizar el curso' })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="create-course-page">
      <div className="create-course-page-header">
        ...
        <h2 className="create-course-page-title">Editar curso</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="create-course-page-form">
        ...
        <div className="login-field">
          <label>Modalidad{hasEnrollments && ' (no editable con inscriptos)'}</label>
          <div className="create-course-modality">
            <button
              type="button"
              className={`create-course-modality-btn ${modality === 'virtual' ? 'active' : ''}`}
              onClick={() => !hasEnrollments && setModality('virtual')}
              disabled={hasEnrollments}
              style={hasEnrollments ? { opacity: 0.5, cursor: 'not-allowed' } : undefined}
            >
            ...
            <button
              ...
              onClick={() => !hasEnrollments && setModality('presencial')}
              disabled={hasEnrollments}
              style={hasEnrollments ? { opacity: 0.5, cursor: 'not-allowed' } : undefined}
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
